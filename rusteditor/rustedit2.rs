        ...
        if let Some(tab) = self.get_active_tab_mut() {
            ...
            if cursor_line < lines.len() {
                ...
                if current_line.trim_start().starts_with(comment_prefix.trim()) {
                    // Uncomment
                    new_lines[cursor_line] = &current_line.replacen(comment_prefix, "", 1);
                } else {
                    // Comment
                    new_lines[cursor_line] = &format!("{}{}", comment_prefix, current_line);
                }
    ...
    fn replace_all(&mut self) {
        if let Some(tab) = self.get_active_tab_mut() {
            if !self.find_text.is_empty() {
                ...
                if self.use_regex {
                    if let Ok(re) = Regex::new(&self.find_text) {
                        tab.content = re.replace_all(&tab.content, &self.replace_text).to_string();
                        tab.is_modified = true;
                    }
                } else {
                    ...
                    if self.case_sensitive {
                        ...
                        while let Some(pos) = lower_content[start..].find(&lower_find) {
                            ...
                            last_end = actual_pos + self.find_text.len();
                            start = last_end;
                        }
    ...
    fn show_menu_bar(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
        egui::menu::bar(ui, |ui| {
            ...
            ui.menu_button("Edit", |ui| {
                ...
                if ui.button("Comment/Uncomment (Ctrl+/)").clicked() {
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Find Next").clicked() && self.total_matches > 0 {
                        ...
                        let matches = self.find_matches();
                        if let Some(tab) = self.get_active_tab_mut() {
                            if let Some(&pos) = matches.get(self.current_match) {
                                tab.cursor_pos = pos;
                                tab.selection_start = Some(pos);
                                tab.selection_end = Some(pos + self.find_text.len());
                            }
                        }
                    }
        ...
        egui::Window::new("About")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.label("Ctrl+/: Comment/Uncomment");
                });
            });
    }
        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                ...
                if total_lines > 0 {
                    ...
                    for (i, chunk) in lines.chunks(step.max(1)).enumerate() {
            ...
            ui.horizontal(|ui| {
                ...
                if self.show_line_numbers {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            ui.style_mut().visuals.override_text_color = Some(self.line_number_color);
                ...
                let text_edit = egui::TextEdit::multiline(&mut tab.content)
                    .font(egui::TextStyle::Monospace)
                    .desired_width(if self.show_minimap { 
