...
impl AlertManager {
    ...
    pub fn check_metric(&self, device_ip: &str, metric: &str, value: f64) {
        if let Some(threshold) = self.thresholds.get(metric) {
            ...
            if self.anomaly_detector.detect_anomaly(device_ip, metric, value) {
                self.create_alert(device_ip, metric, value, "anomaly".to_string(), 0.0);
            }
            if value >= threshold.critical_level {
                self.create_alert(device_ip, metric, value, "critical".to_string(), threshold.critical_level);
            } else if value >= threshold.warning_level {
                self.create_alert(device_ip, metric, value, "warning".to_string(), threshold.warning_level);
            } else {
                // Resolve existing
    ...
    pub fn create_alert(&self, device_ip: &str, metric: &str, value: f64, level: String, threshold: f64) {
        let alert_id = format!("{}_{}_{}", device_ip, metric, Utc::now().timestamp());
        let alert = Alert {
            id: alert_id.clone(),
        ...
        info!("Created alert: {} for device {}", alert_id, device_ip);
    }
...
impl DatabaseManager {
    ...
    pub fn load_devices(&self) -> Result<DashMap<String, NetworkDevice>> {
        let conn = self.connection.lock().unwrap();
        let mut stmt = conn.prepare("SELECT mac, ip, hostname, os_type, status, monitoring_enabled, username, password, bash_history FROM devices")?;
            ...
            rt.block_on(async move {
                while running.load(Ordering::Relaxed) {
                    ...
                    for key in device_keys {
                        if let Some(device_ref) = devices.get(&key) {
                            ...
                            tokio::spawn(async move {
                                ...
                                if ssh.connect_to_device(&mut device_mut).await.is_ok() {
                    ...
                    egui::ScrollArea::vertical().max_height(400.0).show(ui, |ui| {
                        for entry in self.devices.iter() {
                            ...
                            if !self.search_term.is_empty() && 
                               !device_label.to_lowercase().contains(&self.search_term.to_lowercase()) {
                                continue;
                            }
                            
                            let is_selected = self.selected_device.as_ref() == Some(entry.key());
                        ...
                        if let Some(device_ref) = self.devices.get(selected_key) {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                self.modern_gui.create_metric_card(
                                    ...
                                    cpu_latest, 
                                    "%", 
                                    if cpu_latest > 80.0 { egui::Color32::RED } 
                                    else if cpu_latest > 60.0 { egui::Color32::YELLOW }
                                ...
                                self.modern_gui.create_metric_card(
                                    ...
                                    memory_latest, 
                                    "%", 
                                    if memory_latest > 85.0 { egui::Color32::RED }
                                    else if memory_latest > 70.0 { egui::Color32::YELLOW }
                                    else { egui::Color32::GREEN }
                                );
                                ...
                                self.modern_gui.create_metric_card(
                                    ...
                                    disk_latest, 
                                    "%", 
                                    if disk_latest > 90.0 { egui::Color32::RED }
                                    else if disk_latest > 75.0 { egui::Color32::YELLOW }
                                    else { egui::Color32::GREEN }
                                );
                            });
